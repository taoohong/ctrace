...
func newEventPrinter(kind string, relativeTS bool, out io.WriteCloser, err io.WriteCloser) (eventPrinter, error) {
...
func (p tableEventPrinter) Preamble() {
	fmt.Fprintf(p.out, "%-16s %-16s %-6s %-16s %-7s %-7s %-16s %-16s %s", "TIME(s)", "CONTAINER ID", "UID", "COMM", "PID", "TID", "RET", "EVENT", "ARGS")
...
func (p tableEventPrinter) Print(event Event) {
	...
	fmt.Fprintf(p.out, "%-16s %-16s %-6d %-16s %-7d %-7d %-16d %-16s ", timestamp, event.HostName, event.UserID, event.ProcessName, event.ProcessID, event.ThreadID, event.ReturnValue, event.EventName)
	// fmt.Fprintf(p.out, "%-14f %-16s %-6d %-16s %-7d %-7d %-16d %-20s ", event.Timestamp, event.HostName, event.UserID, event.ProcessName, event.ProcessID, event.ThreadID, event.ReturnValue, event.EventName)
